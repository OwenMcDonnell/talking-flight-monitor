...
namespace tfm.PMDG.PMDG737.McpComponents
{
    public partial class AltitudeBox : Form
    {
        ...
        private void AltitudeTimerTick(object Sender, EventArgs eventArgs)
        {
            ...
            if (Aircraft.pmdg737.MCP_annunALT_HOLD.ValueChanged)
            {
                switch (Aircraft.pmdg737.MCP_annunALT_HOLD.Value)
                {
                    case 0:
            ...
            if (Aircraft.pmdg737.MCP_annunVNAV.ValueChanged)
            {
                switch (Aircraft.pmdg737.MCP_annunVNAV.Value)
                {
                    case 0:
            ...
            if (Aircraft.pmdg737.MCP_annunLVL_CHG.ValueChanged)
            {
                switch (Aircraft.pmdg737.MCP_annunLVL_CHG.Value)
                {
                    case 0:
        ...
        {
            altitudeTextBox.Text = Aircraft.pmdg737.MCP_Altitude.Value.ToString();
            if (Aircraft.pmdg737.MCP_annunALT_HOLD.Value == 0)
            {
                altitudeHoldButton.Text = "&Altitude hold off";
                altitudeHoldButton.AccessibleName = "Altitude hold off";
            }
            ...
            if (Aircraft.pmdg737.MCP_annunVNAV.Value == 0)
            {
            ...
            if (Aircraft.pmdg737.MCP_annunLVL_CHG.Value == 0)
            {
                levelChangeButton.Text = "&Level change off";
                levelChangeButton.AccessibleName = "Level change off";
            }
